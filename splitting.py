...
DATA_PATH = 'G:/Full_Dataset/'
FRAME_PATH = DATA_PATH + '/images/'
MASK_PATH = DATA_PATH + '/FASCIA_FINAL/'
...
for i in all_frames:
    j=j+1
    os.rename(FRAME_PATH+i, FRAME_PATH+str(j)+".dcm")

j=0
for i in all_masks:
    j=j+1
    os.rename(MASK_PATH+i, MASK_PATH+str(j)+".png")
'''
all_frames = os.listdir(FRAME_PATH)
all_masks = os.listdir(MASK_PATH)
...
'''
train_masks = [f for f in all_masks if f[:-3]+'dcm' in train_frames]
val_masks = [f for f in all_masks if f[:-3]+'dcm' in val_frames]
test_masks = [f for f in all_masks if f[:-3]+'dcm' in test_frames]
'''
